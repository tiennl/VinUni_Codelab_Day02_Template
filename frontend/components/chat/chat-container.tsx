...
export function ChatContainer() {
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-slate-50/70 dark:bg-slate-950 font-sans overflow-x-hidden">
      {/* Background ambient colorful glow blobs */}
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        <div className="absolute -top-32 -left-32 h-96 w-96 rounded-full bg-cyan-400/15 blur-3xl" />
        <div className="absolute top-1/3 -right-32 h-96 w-96 rounded-full bg-purple-500/15 blur-3xl" />
        <div className="absolute -bottom-32 left-1/3 h-96 w-96 rounded-full bg-pink-500/15 blur-3xl" />
      </div>
      ...
      <main className="relative z-1 flex-1 px-4 py-6 sm:px-6">
        <div className="mx-auto max-w-2xl space-y-6">
          ...
          <div className="text-center py-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-amber-500/10 via-purple-500/10 to-blue-500/10 border border-blue-900/15 px-3.5 py-1 text-xs font-bold text-[#0F2A4A] dark:text-cyan-300 mb-2 shadow-xs backdrop-blur-xs">
              <span>🤖</span> VinSmart AI Assistant
            </span>
            <h2 className="text-2xl font-extrabold text-[#0F2A4A] dark:text-slate-100 tracking-tight">
              Tìm Bất Động Sản <span className="bg-gradient-to-r from-[#0F2A4A] via-indigo-600 to-pink-600 bg-clip-text text-transparent dark:from-cyan-400 dark:via-blue-400 dark:to-pink-400">Vinhomes Ocean Park</span>
            </h2>
          ...
          {state.status === 'loading' && (
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-[#0F2A4A] via-indigo-600 to-pink-500 text-xs font-bold text-white shadow-md shadow-indigo-500/20">
                AI
              </div>
              <TypingIndicator />
            </div>
